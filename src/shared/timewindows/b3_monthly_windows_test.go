...
func TestGenerateB3MonthlyWindows(t *testing.T) {
	...
	t.Run("Cenário referência - 2025-08-14", func(t *testing.T) {
		...
		windows := GenerateB3MonthlyWindows(GenerateB3MonthlyWindowsConfig{
			MinStart: minStart,
			Now:      now,
			Location: loc,
		})
...
func TestHelperFunctions(t *testing.T) {
	...
	t.Run("firstOfMonth", func(t *testing.T) {
		input := time.Date(2023, 5, 15, 14, 30, 45, 123456789, loc)
		expected := time.Date(2023, 5, 1, 0, 0, 0, 0, loc)
		assert.Equal(t, expected, firstOfMonth(input))
	})

	t.Run("endOfDay", func(t *testing.T) {
		input := time.Date(2023, 5, 15, 14, 30, 45, 123456789, loc)
		expected := time.Date(2023, 5, 15, 23, 59, 59, 999999999, loc)
		assert.Equal(t, expected, endOfDay(input))
	})

	t.Run("tern", func(t *testing.T) {
		assert.Equal(t, "yes", tern(true, "yes", "no"))
		assert.Equal(t, "no", tern(false, "yes", "no"))
		assert.Equal(t, 42, tern(true, 42, 24))
		assert.Equal(t, 24, tern(false, 42, 24))
	})
}
