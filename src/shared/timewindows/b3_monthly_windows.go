...
// Window representa uma janela temporal para busca B3
type Window struct {
...
// GenerateB3MonthlyWindowsConfig configuração para geração de janelas
type GenerateB3MonthlyWindowsConfig struct {
...
func GenerateB3MonthlyWindows(config GenerateB3MonthlyWindowsConfig) []Window {
	// Defaults
	loc := config.Location
	if loc == nil {
		var err error
	...
	helpers.LogInfo("Janelas B3 geradas", map[string]interface{}{
		...
		"closedMonths":    len(windows) - tern(len(windows) > 0 && windows[len(windows)-1].IsCurrentMonth, 1, 0),
		"hasCurrentMonth": len(windows) > 0 && windows[len(windows)-1].IsCurrentMonth,
		"firstWindow":     tern(len(windows) > 0, windows[0].Start.Format("2006-01-02 15:04:05"), "none"),
		"lastWindow":      tern(len(windows) > 0, windows[len(windows)-1].End.Format("2006-01-02 15:04:05"), "none"),
	})

	return windows
}

// firstOfMonth retorna o primeiro dia do mês às 00:00:00 na timezone especificada
func firstOfMonth(t time.Time) time.Time {
	return time.Date(t.Year(), t.Month(), 1, 0, 0, 0, 0, t.Location())
}

// endOfDay retorna o final do dia (23:59:59.999999999) na timezone especificada
func endOfDay(t time.Time) time.Time {
	return time.Date(t.Year(), t.Month(), t.Day(), 23, 59, 59, 999999999, t.Location())
}

// tern função ternária genérica
