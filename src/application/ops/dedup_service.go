...
func (s *DedupService) Scan(ctx context.Context, tenantID uuid.UUID, req ScanRequest) (int, error) {
    ...
    if p, err := s.repo.GetPolicy(ctx, tenantID, req.CPF); err == nil && p != nil { policy = *p }

    b3Ops, err := s.repo.ListLedgerOps(ctx, tenantID, req.CPF, "B3_RAW", req.Since, req.To, req.Tickers)
    if err != nil { obs.ObserveDedupScan("error", started); return 0, err }
    ...
    for _, b3 := range b3Ops {
        for _, mo := range manOps {
            if b3.Ticker != mo.Ticker { continue }
            if !withinDays(b3.OperationDate, mo.OperationDate, policy.DateToleranceDays) { continue }
            score, rationale := s.scorePair(b3, mo, policy)
            if score < policy.AlertThreshold { continue }
            cand := DedupCandidate{
                TenantID: tenantID,
                CPF: req.CPF,
            ...
            if !req.DryRun {
                if ok, err := s.repo.UpsertCandidate(ctx, cand); err == nil && ok { created++ } else if err != nil { obs.ObserveDedupScan("error", started); return created, err }
            }
        }
    }
...
func (s *DedupService) Resolve(ctx context.Context, tenantID uuid.UUID, req ResolveRequest) error {
    ...
    case "MERGE":
        prefer := req.Prefer
        if prefer == "" { prefer = "B3_RAW" }
        if err := s.repo.ResolveMerge(ctx, tenantID, req.CandidateIDs, prefer); err != nil { return err }
        obs.IncDedupResolutions("MERGE", len(req.CandidateIDs))
    case "OVERRIDE":
        if err := s.repo.ResolveOverride(ctx, tenantID, req.CandidateIDs); err != nil { return err }
        obs.IncDedupResolutions("OVERRIDE", len(req.CandidateIDs))
    case "IGNORE":
        if err := s.repo.ResolveIgnore(ctx, tenantID, req.CandidateIDs); err != nil { return err }
...
func (s *DedupService) scorePair(b3 LedgerOp, mo LedgerOp, p DedupPolicy) (float64, map[string]interface{}) {
    ...
    sideMatch := 0.0
    if mo.OperationType == b3.OperationType { sideMatch = 1.0 }
    extras := 0.0
...
func zeroIfNil(p *float64) float64 { if p == nil { return 0 } ; return *p }
...
func envOr(def, key string) string { if v := os.Getenv(key); v != "" { return v } ; return def }
func envOrFloat(def float64, key string) float64 { if v := os.Getenv(key); v != "" { if f, err := strconv.ParseFloat(v, 64); err == nil { return f } } ; return def }
