...
func NewManualOperationsService(repo ManualOpsRepository) *ManualOperationsService {
	return &ManualOperationsService{repo: repo}
}
...
func (s *ManualOperationsService) List(ctx context.Context, tenantID uuid.UUID, cpf string, page, pageSize int) ([]ManualOperation, error) {
	return s.repo.ListManual(ctx, tenantID, cpf, map[string]interface{}{}, page, pageSize)
}
