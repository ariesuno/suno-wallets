...
type SystemOpsRepository interface {
	...
	ListOpenInconsistencies(ctx context.Context, tenantID string, cpf string, types []string, tickers []string) ([]Inconsistency, error)
	GetInconsistencyByID(ctx context.Context, tenantID string, id uuid.UUID) (*Inconsistency, error)

	// geração no ledger (idempotente)
	UpsertSystemOperation(ctx context.Context, tenantID string, op OperationPreview) (bool, error)

	// atualização do status da inconsistência e details (anexar generated_op_ids)
	ResolveInconsistency(ctx context.Context, tenantID string, inconsID uuid.UUID, generatedIDs []uuid.UUID) error

	// dados auxiliares
	GetFirstSellTxDetails(ctx context.Context, tenantID string, cpf string, ticker string) (date string, qty float64, price float64, found bool, err error)
	DerivePriceFromPosition(ctx context.Context, tenantID string, cpf, ticker, date string) (price float64, found bool, err error)
}
...
func (s *SystemOperationsService) AutoFix(ctx context.Context, tenantID string, req AutoFixRequest) (*AutoFixResult, error) {
	started := time.Now()
	...
	for _, inc := range incs {
		...
		case "OPENING_BALANCE_MISSING":
			...
			requirePrice := false
			if v := getenv("AUTO_FIX_REQUIRE_PRICE"); v == "true" || v == "1" {
				requirePrice = true
			}
		...
		case "SELL_WITHOUT_BUY":
			...
			if created {
				res.Created++
				obs.IncAutofixOpsCreated("ZERO_PNL_PRE_API", 1)
			} else {
				res.Skipped++
			}
			_ = s.Repo.ResolveInconsistency(ctx, tenantID, inc.ID, nil)
