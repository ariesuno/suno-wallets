...
func NewService(repo Repository, ingest IngestService) *Service { return &Service{repo: repo, ingest: ingest} }
...
func (s *Service) Run(ctx context.Context, p RunParams) (*RunSummary, error) {
    ...
    if p.Scope == "single" {
        ...
        if st != nil { targets = append(targets, *st) }
    } else {
...
func nextTimeOrEpoch(t *time.Time) time.Time {
    if t == nil { return time.Date(1970,1,1,0,0,0,0,time.UTC) }
