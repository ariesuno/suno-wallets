...
func monthWindows(start, end time.Time) [][2]time.Time {
    ...
    for !cur.After(last) {
        ...
        if next.After(end) { next = end }
        winStart := cur
        if winStart.Before(start) { winStart = start }
...
func (s *Service) Ingest(ctx context.Context, p IngestParams) (*Summary, error) {
    ...
    for _, w := range wins {
        ...
        fetch := func(hr *http.Response) (bool, int, error) {
            ...
            if err := json.NewDecoder(hr.Body).Decode(&generic); err != nil { generic = map[string]any{"raw": "decode_error"} }
            h, _ := hashx.ComputeRawHash(generic, path, p.TenantID, p.CPF, baseQuery["referenceStartDate"], baseQuery["referenceEndDate"], 0)
            ...
            rec := &persistence.RawRecord{ID: uuid.New(), TenantID: uuid.MustParse(p.TenantID), CPF: p.CPF, DataType: p.DataType, AssetType: p.AssetType,
                PeriodStart: w[0], PeriodEnd: w[1], Page: 0, PayloadJSON: payload, PayloadHash: h, SourceVer: map[bool]string{p.DataType=="positions": "v3"}[true],
