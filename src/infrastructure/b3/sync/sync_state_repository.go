...
type Repository interface {
	GetActiveByTenant(ctx context.Context, tenantID string, limit int) ([]SyncState, error)
	GetByTenantCPF(ctx context.Context, tenantID string, cpf string) (*SyncState, error)
...
func (r *repositoryImpl) GetActiveByTenant(ctx context.Context, tenantID string, limit int) ([]SyncState, error) {
	var out []SyncState
...
func (r *repositoryImpl) GetByTenantCPF(ctx context.Context, tenantID string, cpf string) (*SyncState, error) {
	var st SyncState
	if err := r.db.WithContext(ctx).Raw(`SELECT * FROM b3_sync_state WHERE tenant_id = ? AND cpf = ?`, tenantID, cpf).Scan(&st).Error; err != nil {
		return nil, err
	}
	if st.ID == uuid.Nil {
		return nil, nil
...
func (r *repositoryImpl) MarkResult(ctx context.Context, id uuid.UUID, success bool, needsReprocess bool, lastResult string, lastError *string, txSyncAt, posSyncAt *time.Time) error {
	...
	if success {
		if txSyncAt != nil {
			set += ", last_tx_sync_at = ?"
			args = append(args, *txSyncAt)
		}
		if posSyncAt != nil {
			set += ", last_pos_sync_at = ?"
			args = append(args, *posSyncAt)
		}
		set += ", failure_count = 0"
	} else {
