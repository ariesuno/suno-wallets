...
type NormalizedTransaction struct {
	...
	MarketName              *string    `gorm:"type:text"`
	ParticipantName         *string    `gorm:"type:text"`
	ParticipantDocument     *string    `gorm:"type:text"`
	AssetTradingCode        *string    `gorm:"type:text"`
	ExpirationDate          *time.Time `gorm:"type:date"`
	OptionExerciseValue     *string    `gorm:"type:numeric"`
	OriginalTradePrice      *string    `gorm:"type:numeric"`
	OriginalAdjustmentValue *string    `gorm:"type:numeric"`
	TradeDateTime           *time.Time `gorm:"type:timestamptz"`
	...
	for _, it := range items {
		if err := r.db.WithContext(ctx).Exec(`
            INSERT INTO b3_normalized_transactions (
                ...
                trade_date, settlement_date, ticker, isin, side, quantity, price, gross_value, currency,
                market_name, participant_name, participant_document, asset_trading_code, expiration_date,
                option_exercise_value, original_trade_price, original_adjustment_value, trade_datetime,
        ...
        `,
			...
			it.TradeDate, it.SettlementDate, it.Ticker, it.ISIN, it.Side, it.Quantity, it.Price, it.GrossValue, it.Currency,
			it.MarketName, it.ParticipantName, it.ParticipantDocument, it.AssetTradingCode, it.ExpirationDate,
			it.OptionExerciseValue, it.OriginalTradePrice, it.OriginalAdjustmentValue, it.TradeDateTime,
	...
	for _, it := range items {
		if err := r.db.WithContext(ctx).Exec(`
            INSERT INTO b3_normalized_positions (
                ...
                quantity, avg_price, position_value, currency, normalized_hash, normalized_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT (tenant_id, cpf, asset_type, raw_id, sequence_in_raw, normalized_hash)
            DO UPDATE SET normalized_at = EXCLUDED.normalized_at
        `,
			it.ID, it.TenantID, it.CPF, it.AssetType, it.SourceVersion, it.RawID, it.SequenceInRaw, it.ReferenceDate, it.Ticker, it.ISIN,
			it.Quantity, it.AvgPrice, it.PositionValue, it.Currency, it.NormalizedHash, time.Now(),
		).Error; err != nil {
			return err
		}
	}
