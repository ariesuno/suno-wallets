...
func (r *OperationsRepository) ListManual(ctx context.Context, tenantID uuid.UUID, cpf string, filters map[string]interface{}, page, pageSize int) ([]appops.ManualOperation, error) {
	...
	if err := qb.Order("operation_date DESC").Limit(pageSize).Offset((page - 1) * pageSize).Scan(&out).Error; err != nil {
		return nil, err
	}
	return out, nil
}
...
func (r *OperationsRepository) GetPolicyMode(ctx context.Context, tenantID uuid.UUID, cpf string) (string, error) {
	// Primeiro tenta em client_data_source_policy
	type row struct{ Mode string }
	var rw row
	if err := r.db.WithContext(ctx).Raw(`SELECT mode FROM client_data_source_policy WHERE tenant_id = ? AND cpf = ? LIMIT 1`, tenantID, cpf).Scan(&rw).Error; err == nil && rw.Mode != "" {
		return rw.Mode, nil
	}
	// Fallback: dedup_policies.mode (legado)
	rw = row{}
	if err := r.db.WithContext(ctx).Raw(`SELECT COALESCE(mode,'HYBRID') AS mode FROM dedup_policies WHERE tenant_id = ? AND cpf = ? LIMIT 1`, tenantID, cpf).Scan(&rw).Error; err == nil && rw.Mode != "" {
		return rw.Mode, nil
	}
	return "HYBRID", nil
}
