...
func SetupRoutes(cfg *config.Config, db *gorm.DB) *gin.Engine {
	...
	// Complete Sync Orchestrator (utilizado pelo backoffice)
	completeSyncOrch := appsync.NewCompleteSyncOrchestrator(reactivationSvc, e2eOrch, incrementalSvc, reconSvc, syncRepoIncr)
	// Backoffice Admin (1.22) - com todos os orchestrators conectados
	b3SyncOrch := adminrepo.NewB3SyncOrchestratorAdapter(e2eOrch, incrementalSvc, completeSyncOrch)
	reconOrch := adminrepo.NewReconciliationOrchestratorAdapter(reconSvc)
	dedupOrch := adminrepo.NewDedupOrchestratorAdapter(dedupSvc)
	policyOrch := adminrepo.NewPolicyOrchestratorAdapter(polSvc)
	clientDataOrch := adminrepo.NewClientDataOrchestratorAdapter(e2eOrch)
	admRepo := adminrepo.NewRepository(db, b3SyncOrch, reconOrch, dedupOrch, policyOrch, clientDataOrch)
	...
	// Complete Sync Orchestrator (fluxo completo inteligente) - usando instância já criada
	...
	apiV1.Use(middlewares.TenantMiddleware())
	{
		...
		admin.GET("/backoffice/export/ledger/csv", backofficeController.ExportLedgerCSV)
		...
		// Ações específicas (padrão 2 passos: solicitar + confirmar)
		admin.POST("/backoffice/actions/b3-full-fetch", backofficeController.B3FullFetch)
		admin.POST("/backoffice/actions/b3-full-fetch/confirm", backofficeController.ConfirmB3FullFetch)
		admin.POST("/backoffice/actions/b3-incremental", backofficeController.B3Incremental)
		admin.POST("/backoffice/actions/b3-incremental/confirm", backofficeController.ConfirmB3Incremental)
		admin.POST("/backoffice/actions/recon-scan", backofficeController.ReconScan)
		admin.POST("/backoffice/actions/recon-scan/confirm", backofficeController.ConfirmReconScan)
		admin.POST("/backoffice/actions/auto-fix", backofficeController.AutoFix)
		admin.POST("/backoffice/actions/auto-fix/confirm", backofficeController.ConfirmAutoFix)
		admin.POST("/backoffice/actions/dedup-scan", backofficeController.DedupScan)
		admin.POST("/backoffice/actions/dedup-scan/confirm", backofficeController.ConfirmDedupScan)
		admin.POST("/backoffice/actions/dedup-resolve", backofficeController.DedupResolve)
		admin.POST("/backoffice/actions/dedup-resolve/confirm", backofficeController.ConfirmDedupResolve)
		admin.POST("/backoffice/actions/policy-set", backofficeController.PolicySet)
		admin.POST("/backoffice/actions/policy-set/confirm", backofficeController.ConfirmPolicySet)
		admin.POST("/backoffice/actions/client-reset", backofficeController.ClientReset)
		admin.POST("/backoffice/actions/client-reset/confirm", backofficeController.ConfirmClientReset)
		admin.POST("/backoffice/actions/zero-and-refetch", backofficeController.ZeroAndRefetch)
		admin.POST("/backoffice/actions/zero-and-refetch/confirm", backofficeController.ConfirmZeroAndRefetch)
